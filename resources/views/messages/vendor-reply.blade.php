...
<div class="bg-white/70 backdrop-blur-md rounded-2xl shadow-lg border border-white/40 p-6 max-w-2xl mx-auto">
    <h1 class="text-xl font-bold text-gray-800 mb-1">{{ $customer->name }}</h1>
    <p class="text-xs text-gray-500 mb-4">{{ $customer->phone }}</p>
    ...
    <form method="POST" action="{{ route('messages.vendor.storeAs', $customer) }}" class="flex gap-2">
        @csrf
        <input type="text" name="body" placeholder="Type a reply..." class="flex-1 border rounded-lg p-2 text-sm" required>
