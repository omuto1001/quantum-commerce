...
<div class="bg-white/70 backdrop-blur-md rounded-2xl shadow-lg border border-white/40 p-6 max-w-2xl mx-auto">
    <h1 class="text-xl font-bold text-gray-800 mb-1">{{ $vendor->shop_name }}</h1>
    <p class="text-xs text-gray-500 mb-4">Ask a question before you order</p>
    ...
    <form method="POST" action="{{ route('messages.vendor.store', $vendor) }}" class="flex gap-2">
        @csrf
